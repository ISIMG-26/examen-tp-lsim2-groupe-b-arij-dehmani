...
function saveCart() {
    ...
    if (t) t.textContent = (cart.reduce((s, i) => s + i.prix * i.quantite, 0)).toFixed(2) + ' TND';
}
...
async function loadCats(barId) {
    ...
    el.querySelectorAll('.cat-btn').forEach((b) => b.addEventListener('click', async () => {
        el.querySelectorAll('.cat-btn').forEach((x) => x.classList.remove('active'));
        b.classList.add('active');
        activeCat = b.dataset.id || '';
        await loadProds(activeCat, document.getElementById('search-input')?.value.trim() || '');
    }));
}
...
async function loadProds(cat = '', query = '') {
    ...
    g.innerHTML = d.produits.map((p) => {
        ...
        return `<div class="product-card">${imgHtml}<div class="product-body">
            ...
            <div class="product-footer"><span class="product-price">${parseFloat(p.prix).toFixed(2)} TND</span>
